...
namespace SurveyBuilder.Application.Validators.AccountCategory
{
    public class UpdateAccountCategoryDTOValidation:AbstractValidator<UpdateAccountCategoryDTO>
    {
        public UpdateAccountCategoryDTOValidation()
        {
            RuleFor(x => x.Id).NotNull().WithMessage("Id is required.")
                .GreaterThan(0)
                .WithMessage("Id must be greater than 0.");
            RuleFor(x => x.Name).NotEmpty()
                            .WithMessage("Please enter account category name.")
                            .MaximumLength(255)
